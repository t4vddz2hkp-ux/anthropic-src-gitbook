# Anthropic `src` 源码教科书（学生自学深度研究版）

> 面向系统自学、源码深读与长期掌握的 GitBook 教材  
> 分析对象：`src`

## 本版定位

这是“学生自学深度研究版”教材。它的目标不只是让读者“知道这个系统大概做了什么”，而是让学生在完成全书之后，能够真正具备下面四种能力：
...
因此，这一版不再把外部使用场景当成中心，而是把“学生如何从零开始，最后真正掌握它”当成中心来组织内容。全书当前共 30 章、8000 行左右正文，采用“先建地图，再走主线，后做模块精读，最后以训练和考核收束”的结构。

这一版的重组不是简单地在原书后面增加几章附录，而是直接改动了教材的组织逻辑：

- 目录顺序改成“阶段式自学路径”，而不是“作者想到什么就排什么”。
- 核心章节会明确告诉学生“这一章在整个学习路线中的位置”。
- 模块地图不再只按目录平铺，而是按“先读什么、后读什么、哪些暂时别碰”来组织。
- 代码阅读指南不再只是建议，而是变成一套可执行的自学方法。
- 所有章节都只保留学生真正需要的内容，不再混入与主学习路径无关的附加说明。
- 在主干目录里插入了术语词典、请求时序剖面、工具执行闭环、重量级能力案例、状态与恢复全链路等“深度研究章节”。

同时，这一版已经不再是“部分章节采用学生视角、部分章节仍是旧讲义口吻”的混合状态，而是把全书各章都统一改造成了同一类写法：

- 先说明本章在全书或学习路线中的位置。
- 再指出学生最容易误解、最容易卡住的地方。
- 再给出本章的阅读抓手、观察线或使用方式。
- 最后再进入具体源码、数据结构、流程和案例分析。

## 本书采用的教材写法

这次重写并不是随意改口吻，而是吸收了通行教材设计里几条非常稳定的方法，然后再按这套源码的实际结构做了本地化改造：

- 先定义目标读者和学习结果，再决定目录顺序。换句话说，先想“学生学完要会什么”，再想“章节怎么排”。
- 章节结构保持稳定。多数章节都会交代本章位置、常见误区、阅读抓手、核心展开、小结和思考题，降低大型工程阅读的认知切换成本。
- 先搭支架，再讲细节。对零基础读者，先给术语、类比、全局图和主链路，再进入具体文件和复杂分支。
- 例子、图和练习嵌在正文里，而不只放在章节末尾。这样读者读到抽象概念时，马上就能看到它如何落到实际源码。
- 正文只保留学习者需要的内容，不把额外的组织安排或说明性材料混进主线。
...
这不是一份“文件注释汇编”，而是一份真正面向学生自学的源码教材。它试图回答四个问题：
...
6. 通关训练阶段：先用第 22 章做整书复盘，再完成第 24、25 章里的自学任务、实验题和自测问题。
...
flowchart TD
    ...
    J --> K["第 24 章 + 第 25 章<br/>自学通关与综合训练"]
```
