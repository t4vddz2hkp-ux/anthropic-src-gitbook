...
## 先修关系
- 建议先读第 4 章和第 1 章，至少要知道这是 REPL 中的 Agent 平台。
- 如果第 2 章术语表已经浏览过，本章里的消息、tool_use、assistant block 会更容易吸收。
- 本章是第 8、9、11、10、14 章的总入口，读完后再回去拆各子模块会更轻松。
...
## 重建蓝图：把一次请求整理成可落地的交易流水线

### 必须保留的抽象

1. 一次请求应被视为一笔长事务，而不是简单的“输入一次、输出一次”。它包含输入接入、消息正规化、上下文装配、模型流式生成、工具回环、UI 投影与持久化收尾等多个阶段。
2. 每个阶段都应有清晰的输入产物和输出产物。例如 `processUserInput()` 的输出是 canonical message，`QueryEngine.submitMessage()` 的输出是一次可驱动查询状态机的任务。
3. 阶段之间应通过显式 handoff 数据连接，而不是依赖隐式的全局变量。这样才能在其他语言中重新搭出同样的链路。
4. 工具回环必须被视为该事务内部的再入过程，而不是事务外的旁路调用。

### 最小实现顺序

1. 先把 14 个阶段压缩成一个阶段表，并给每个阶段定义“开始条件、结束条件、核心数据、失败分支”。
2. 第二步实现阶段间日志，使系统能够记录每次请求当前位于哪一阶段。没有这层可观测性，后续调试和恢复都会很困难。
3. 第三步按阶段逐个实现，从 REPL 接收输入开始，到 transcript 落盘结束，每一步都先用假数据通路验证。
4. 第四步在事务中插入工具回环和后台任务分支，确保同一请求可以多次经过“模型 -> 工具 -> 模型”的闭环。
5. 第五步再实现取消、中断、重试与恢复，使事务能够在异常情况下仍保持一致性。

### 最容易写错的边界

1. 最常见的错误是只关注最终回答文本，而忽略事务中间产物。实际上 transcript、tool_result、usage 统计、状态变化都属于请求事务的重要结果。
2. 第二类错误是没有明确工具结果回写的阶段位置，导致结果先显示在 UI，后写入消息流，最终出现顺序漂移。
3. 第三类错误是把持久化放到最后一刻集中处理。原工程之所以在早期就写用户消息 transcript，是为了在请求尚未完成时也能保证恢复语义。
4. 第四类错误是没有把失败分支纳入时序图，导致一旦 API 失败、权限拒绝或任务后台化，链路描述就失效。
...
- 如后续在 第 8 章、第 9 章、第 11 章和第 10 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 8 章、第 9 章、第 11 章和第 10 章，本章哪一部分最值得先回看？为什么？
