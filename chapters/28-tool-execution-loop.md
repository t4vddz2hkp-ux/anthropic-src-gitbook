...
## 先修关系
- 建议先读第 7 章和第 8 章，先知道 Tool 协议长什么样以及 BashTool 为什么复杂。
- 如果第 6 章和第 19 章已经读过，你会更容易把工具执行放回 query 与流式返回之间。
- 本章与第 29 章互为专题深化：这里讲闭环，那里讲重量级能力横向比较。
...
## 章末小结
- 本章围绕“工具执行闭环、并发安全与结果回写”重建了一层稳定理解，不让你只记零散函数名或目录名。
...
- 如果你后续在 第 29 章和第 21 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 29 章和第 21 章，本章哪一部分最值得先回看？为什么？
