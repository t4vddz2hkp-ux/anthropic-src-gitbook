# 第 10 章 把一次请求拆成 14 个阶段：从输入到 `tool_result` 的时序剖面

## 本章目标
- 把一次请求从进入系统到得到 `tool_result` 的全过程拆成稳定阶段。
- 训练对时序、阶段边界和阶段输出的敏感度。
- 以便在后续调试或讲解时，能够准确指出“问题出在哪一阶段”。

## 先修关系
- 建议先读第 5 章和第 9 章，以先建立对整体主链路的感觉。
- 如果第 11 章和第 14 章读过，将更容易看懂时间线中 API 流和工具流的切换。
- 本章最好和第 22 章搭配读，因为很多阶段边界同时也是持久化或恢复边界。
...
| --- | --- | --- |
| `14 阶段总表` | 整章的核心时序索引。 | 它有助于迅速判断某个现象落在哪一步。 |
| `Stage Input/Output` | 每一阶段都有自己的输入、产物和下一跳。 | 这能把时间线从故事变成结构。 |
| `tool_use Event` | 模型流里切换到工具执行的关键结构。 | 没有它，主循环不会进入工具支路。 |
| `Compact / Transcript Boundary` | 某些阶段同时也是语义持久化的边界。 | 它有助于理解恢复与时序的关系。 |

## 本章在学习路线中的位置

这一章属于第二阶段，也就是“走通主链路”的强化版。  
如果第 5 章像一次全景式导览，那么这一章就是把那条链路拆得更细，细到读者可以真正拿着源码逐步对照。

本章的目标不是继续增加概念，而是把下面的问题真正回答清楚：

> 用户输入一句话以后，系统到底先做什么、后做什么、在哪里切层、什么时候进入工具、什么时候又回到模型？

## 为什么必须把“一次请求”拆成阶段

很多读者会把一次请求想象成下面这种伪代码：
...
这一步非常适合拿来给读者说明“工程系统和 demo 程序的区别”：
...
如果这五题都能答清，说明主链路已经不只是“有印象”，而是进入了真正可追踪的状态。

## 重建蓝图：把请求时间线写成阶段状态机

### 必须保留的抽象

1. 14 个阶段不应只是说明顺序的目录，而应被当作请求级状态机的阶段定义。每一阶段都应说明进入条件、离开条件、产生的工件和可能的异常分支。
2. 阶段之间的工件交接必须明确，例如消息对象、QueryConfig、API request、流式事件、tool_result、transcript record、状态快照。
3. 时间线应兼容再入与循环。一个请求并不是线性前进到结束，工具执行后可能返回到新的模型阶段，因此时间线本身要允许回环。
4. 时间线还应兼容并行旁支，例如后台任务登记、通知更新、状态同步与 UI 重绘，它们虽然不改变主阶段顺序，却会围绕阶段发生。

### 最小实现顺序

1. 先为每个阶段编号并定义名称，使日志与测试都能引用统一阶段代码。
2. 第二步为每个阶段列出输入工件与输出工件，并在实现中强制经过这些 handoff。
3. 第三步为关键阶段增加可观测日志或事件，让系统能够报告“当前请求处于第几阶段”。
4. 第四步补入回环与失败分支，使时间线不仅能描述 happy path，也能描述工具调用、权限拒绝、中断和恢复。

### 最容易写错的边界

1. 最常见的错误是把某些阶段合并得过早，例如把上下文装配、transcript 记录和 API 调用视为一个黑盒步骤。这样一旦要做恢复或调试，就会找不到边界。
2. 第二类错误是时间线没有显式工件，因此只能描述“大概做了什么”，却无法支撑重写。
3. 第三类错误是没有把循环结构写进时间线，导致工具回环在模型输出后显得像异常特判。
4. 第四类错误是没有为时间线定义异常出口，结果所有失败都只能以“请求失败”笼统收尾。

## 章末小结
- 本章围绕“阶段化时间线、阶段边界与问题定位”重建了一层稳定理解，避免只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `阶段`、`前置准备`、`Query 初始化` 这几个词，而是它们在 `14 阶段总表`、`Stage Input/Output`、`tool_use Event` 里的相互位置。
- 如后续在 第 11 章、第 14 章和第 22 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 11 章、第 14 章和第 22 章，本章哪一部分最值得先回看？为什么？
