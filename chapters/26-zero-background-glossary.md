...
- 如果第 15 章已经读过，你会更容易把这里的术语精确定义和生活化理解对应起来。
...
这一章放在第一阶段，不是为了“凑词典”，而是为了帮没有背景的学生拆掉第一个阅读障碍：  
很多代码根本不是难在逻辑，而是难在名词。你一旦把名词听成了模糊的大概意思，后面的理解会层层偏掉。
...
最容易理解的单位就是“进程”。  
当你启动这个程序时，操作系统会创建一个运行中的进程。这个进程里会加载 `main.tsx`、初始化环境、构建状态、进入 REPL。
...
> 从你打开终端助手，到这段工作结束并可被恢复，这整个过程就是一个 session。
...
`QueryEngine` 不是“查询数据库的引擎”，也不是“帮你搜资料的模块”。  
它在本系统中的真实身份更接近：
...
它告诉你一个 QueryEngine 要想工作，需要拿到哪些外部条件，例如：
...
它告诉你具体工具在执行时拿到的是怎样一个运行现场。

### 4. AppState

它告诉你 UI、任务、MCP、插件、通知、权限模式、模型设置等大量状态在应用层是怎么集中组织的。
...
## 章末小结
- 本章围绕“术语精确定义、混淆边界和随查能力”重建了一层稳定理解，不让你只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `词条`、`概念分组`、`混淆对` 这几个词，而是它们在 `Glossary Entry`、`Concept Group`、`Confusion Pair` 里的相互位置。
- 如果你后续在 第 15 章和任何你正在读的主线章节 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 15 章和任何你正在读的主线章节，本章哪一部分最值得先回看？为什么？
