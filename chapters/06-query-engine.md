...
- 如果第 2 章和第 3 章已经读过，你会更容易把 QueryEngine 放回装配层与运行层的边界上。
...
## 章末小结
- 本章围绕“会话执行器、单轮状态机和配置/状态分离”重建了一层稳定理解，不让你只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `QueryEngine`、`query()`、`QueryConfig` 这几个词，而是它们在 `QueryEngine 上下文`、`QueryConfig`、`State` 里的相互位置。
- 如果你后续在 第 19 章、第 27 章和第 28 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
