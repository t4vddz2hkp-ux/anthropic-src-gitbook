...
- 让读者形成“如何评价一个重量级系统能力”的观察框架。

## 先修关系
- 建议先读第 8、9、10、28 章，否则本章的比较会缺少具体参照。
- 如果第 7 章已经很熟，你会更容易看出三者在同一 Tool 协议之下的异构性。
- 本章适合作为第三阶段收束，帮助你从单点模块跳到横向能力视角。
...
| `Risk Matrix` | 从安全、权限、远程依赖等角度比较能力风险。 | 它帮助你理解为什么治理策略不能一刀切。 |
...
如果你只看 FileRead、Glob 之类轻量工具，会觉得这不过是一套方便的工具总线。  
但一旦把这三类能力看懂，你就会意识到：  
...
语言模型最强的一点，是生成决策和指令。  
但如果系统只能回答“我建议你执行这个命令”，而不能真正执行，很多自动化价值就出不来。
...
很多学生第一次看到 `AgentTool` 会说：“这不就是再调用一次模型吗？”  
这其实只说对了一半。
...
光从这些字段你就能看出，它已经不像“普通工具调用”，而像一个迷你任务调度系统。
...
## 章末小结
- 本章围绕“重量级能力比较、风险画像与生命周期差异”重建了一层稳定理解，不让你只记零散函数名或目录名。
