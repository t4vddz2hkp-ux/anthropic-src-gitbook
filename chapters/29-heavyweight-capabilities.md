# 第 18 章 三大重量级能力专题：BashTool、AgentTool 与 MCP
...
## 先修关系
- 宜先读第 15、16、17、14 章，否则本章的比较会缺少具体参照。
- 如果第 13 章已经很熟，将更容易看出三者在同一 Tool 协议之下的异构性。
...
## 重建蓝图：把三类重能力归并为统一能力家族

### 必须保留的抽象

1. BashTool、AgentTool 与 MCP 虽然差异很大，但在平台层都应表现为“可发现、可调度、受权限与上下文约束的重能力”。这种统一外观是跨语言重写的关键。
2. 三者的主要差异不在外层协议，而在内部调度器和资源边界。Bash 面向外部进程，Agent 面向子会话与任务，MCP 面向远程 server 与 transport。
3. 因此重写时应优先抽出共同的 Capability Envelope，再分别实现三种专用执行后端。
4. 每种重能力都必须有独立的生命周期管理：进程生命周期、任务生命周期、连接生命周期。只有这样，系统才能长期稳定运行。

### 最小实现顺序

1. 先实现最容易验证的 BashTool，建立外部动作、权限和结果回写的基本闭环。
2. 第二步在此基础上实现 AgentTool，把“能力触发新会话”的模式引入系统。
3. 第三步再实现 MCP，把远程能力发现、认证、重连和动态汇入纳入统一平台。
4. 第四步用统一的能力矩阵检查三者是否都满足名称发现、schema、权限、生命周期、结果信封与 transcript 规则。

### 最容易写错的边界

1. 最常见的错误是把三类能力做成三套彼此独立的平台，结果主循环、权限和状态同步都要分别适配三遍。
2. 第二类错误是只关注成功调用，而忽略重能力各自的资源清理。进程僵死、任务泄漏、连接失效都会直接损害长期运行稳定性。
3. 第三类错误是把 AgentTool 或 MCP 过早特化到某个供应商或宿主环境，从而失去抽象层统一性。
4. 第四类错误是没有统一错误语义，使 Bash、Agent、MCP 各自抛出不同风格的错误，迫使主循环理解三套异常模型。
