...
- 如果你已经看过第 15 章，看到 REPL、会话环境、信任流程会更自然。
- 本章最好和第 16 章对照着读，这样你能同时看到“启动前”和“请求时”的区别。
...
| `惰性导入点` | 真正重量级模块不会在顶层全部拉进来。 | 它能帮助你理解启动优化和耦合控制。 |
...
## 章末小结
- 本章围绕“启动链、预热策略和安全边界”重建了一层稳定理解，不让你只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `main.tsx`、`init`、`setup` 这几个词，而是它们在 `启动链`、`安全配置阶段`、`会话环境对象` 里的相互位置。
- 如果你后续在 第 4 章和第 16 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
