# 第 20 章 模块学习地图：按阅读优先级解读目录

## 本章目标
- 把庞大的顶层目录重新整理成“现在先读什么、以后再查什么”的学习地图。
- 让读者在迷路时能够快速回到正确目录，而不是在 `utils/` 或 `components/` 里漫游。
- 建立目录查阅能力，让“知道结构”升级成“知道从哪一层重新切入”。

## 先修关系
- 建议至少已经读过第 2、16、17、19 章中的若干核心章节，否则本章会太像百科全书。
- 本章最适合在第一轮主线阅读后使用，用来补齐“覆盖面”和“目录感”。
- 如果你发现自己总是找不到对应文件，本章就是应该回来的地方。

## 关键词
- `学习优先级`：不是所有目录都值得立刻读，必须按理解收益排序。
- `模块地图`：把顶层目录看成结构地图而不是线性目录清单。
- `主骨架目录`：支撑你建立系统地图的第一优先级目录。
...
所以，本章不再把目录当成一份简单词典，而是先给出一张“读者该先读什么、后读什么”的学习地图。下文保留目录词典式说明，但那一部分更适合在你已经建立地图之后作为查阅工具使用。
...
很多读者第一轮只盯着 `query.ts`，结果后面一问“用户输入怎么分流”“状态是怎么同步的”“工具到底是谁在调度”，就答不上来了。  
...
这一层目录在工程里当然有价值，但对第一轮读者来说，不应该作为主要学习入口。  
...
## 章末小结
- 本章围绕“学习优先级、目录感和模块查阅能力”重建了一层稳定理解，不让你只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `学习优先级`、`模块地图`、`主骨架目录` 这几个词，而是它们在 `优先级总表`、`Atlas Entry`、`核心文件集合` 里的相互位置。
- 如果你后续在 第 21 章、第 24 章和后续源码实践 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 21 章、第 24 章和后续源码实践，本章哪一部分最值得先回看？为什么？
