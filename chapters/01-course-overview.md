...
## 先修关系
- 可以直接开始；如果你对术语比较陌生，建议先快速浏览第 15 章和第 26 章。
- 如果你已经读过第 23 章，本章会帮你把路线图重新压缩成“为什么要这样学”的解释。
- 读本章时不要求代码背景，但建议顺手记下三到五个你最陌生的词，后面会不断回收。
...
| --- | --- | --- |
| `系统定位句` | 用一句话回答这套 `src` 到底在构建什么。 | 它决定你后续是把项目当脚本读，还是当平台读。 |
| `五类复杂性清单` | 把阅读难点分成产品、运行时、UI、扩展和治理五类。 | 后面几乎所有难点都能回到这五类来源。 |
| `目录统计表` | 用文件量和目录分布建立规模感。 | 它能提醒你不要用小项目的阅读习惯处理大工程。 |
| `六阶段学习路线` | 把全书内容按依赖顺序重新排列。 | 它是你后续自学、复习和回看的总导航。 |
...
## 章末小结
- 本章围绕“系统定位、复杂性来源和全书学习路线”重建了一层稳定理解，不让你只记零散函数名或目录名。
- 真正需要沉淀下来的，不只是 `Agent 平台`、`REPL`、`主链路` 这几个词，而是它们在 `系统定位句`、`五类复杂性清单`、`目录统计表` 里的相互位置。
- 如果你后续在 第 2 章和第 16 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 2 章和第 16 章，本章哪一部分最值得先回看？为什么？
