...
## 先修关系
- 建议先读第 13 章，至少要知道 Tool 协议的公共外壳。
- 如果第 9 章已经读过，将更容易把 BashTool 放回 query 主循环里理解。
- 本章最好与第 14 章和第 18 章配合阅读，前者看闭环，后者看重量级能力对比。
...
如果第 13 章讲的是“工具协议的抽象层”，那么这一章讲的就是“一个重量级工具怎样把抽象层全部用满”。  
...
## 重建蓝图：把 BashTool 还原成受监管的外部进程能力

### 必须保留的抽象

1. BashTool 的本质不是“执行一条 shell 命令”，而是“把模型给出的意图翻译为受约束的外部进程调用”。因此至少要保留命令请求对象、安全判定结果、执行模式、结果信封四类抽象。
2. 执行模式必须至少区分前台与后台。原工程中的 `run_in_background`、自动后台化和 `backgroundTaskId` 并不是装饰字段，而是用来把长任务从即时响应路径中剥离出去。
3. 安全判断必须单独建模。`bashPermissions.ts` 中的权限检查和推测性分类器检查表明，是否允许运行某条命令属于平台政策，不属于 shell runner 本身。
4. BashTool 与 `LocalShellTask` 的分离也必须保留。前者是模型可见能力，后者是长期运行任务实体；一个负责“发起”，一个负责“持续存在”。

### 最小实现顺序

1. 先定义 BashTool 的输入 schema，包括命令文本、工作目录、超时策略、是否后台化以及可选的环境控制字段。
2. 第二步实现命令执行器，但暂时只支持前台阻塞执行。此时要先把 stdout、stderr、退出码和截断策略封装成统一结果结构。
3. 第三步加入权限与风险判定，把“命令是否合法、是否需要确认、是否禁止运行”从 shell 调用逻辑中拆出来。
4. 第四步实现后台任务登记簿，引入任务 ID、状态更新、进度摘要和结果回收，使长命令不再阻塞主循环。
5. 第五步再补入自动后台化、并发安全判断、终端 UI 提示与更细的环境隔离能力。

### 最容易写错的边界

1. 最常见的错误是在工具 handler 内直接 `spawn` 并同步等待全部结束。这样做无法支持后台任务，也无法把执行生命周期暴露给系统其他部分。
2. 第二类错误是将 shell 输出原样无限制写回消息流。原工程之所以有结果大小治理和任务面板，是因为命令输出很容易吞没对话上下文。
3. 第三类错误是忽略并发安全。某些命令可以并行执行，某些命令会互相污染工作区或占用终端资源，`isConcurrencySafe` 一类判断必须在设计早期纳入。
4. 第四类错误是把权限确认视为 UI 细节。实际上权限模式影响主循环决策、任务调度和 transcript 记录，是系统级约束。
...
- 如后续在 第 14 章和第 18 章 中再次迷路，优先回看本章的“先修关系、正文图解、关键数据结构”三部分。
...
5. 如果后续要继续读 第 14 章和第 18 章，本章哪一部分最值得先回看？为什么？
