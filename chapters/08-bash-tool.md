...
- 如果第 6 章已经读过，你会更容易把 BashTool 放回 query 主循环里理解。
...
对读者来说，BashTool 是最能帮助你把协议、权限、任务、输出治理和 UI 展示一次性串起来的案例。
